...
const createCollection = async (
  ...
  try {
    const res = await db.createCollection(ASTRA_DB_COLLECTION_3, {
      vector: {
...
const loadSampleData = async () => {
  try {
    const collection = await db.collection(ASTRA_DB_COLLECTION_3);
    ...
    const schemaQuery =
      "SELECT column_name, data_type FROM information_schema.columns WHERE table_name = 'points_of_interests'";
    ...
    const sourceQuery = "SELECT name, description FROM points_of_interests";
    ...
    if (records.length > 0) {
      ...
      console.warn("No records found in points_of_interests table");
      return;
    }

    // Pentru fiecare înregistrare, procesăm name și description
    let recordIndex = 1; // Înlocuitor pentru 'id'
    for (const record of records) {
      const { name, description } = record;
      console.log(`Processing record ${recordIndex}:`, { name, description });

      // Validăm datele
      if (!name && !description) {
        console.warn(
          `Skipping record ${recordIndex}: both name and description are empty`
        );
        recordIndex++;
        continue;
      }

      // Creăm un text descriptiv care include name și description
      const textToProcess = `${name || "Point of Interest"} - ${
