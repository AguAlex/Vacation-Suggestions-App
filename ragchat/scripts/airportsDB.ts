...
const createCollection = async (
  ...
  try {
    const res = await db.createCollection(ASTRA_DB_COLLECTION_4, {
      vector: {
...
const loadSampleData = async () => {
  try {
    const collection = await db.collection(ASTRA_DB_COLLECTION_4);
    ...
    const sourceQuery = "SELECT name, detailed_name FROM airports";
    ...
    if (records.length > 0) {
      ...
      console.warn("No records found in airports table");
      return;
    }

    // Pentru fiecare înregistrare, procesăm name, detailed_name, și iataCode
    let recordIndex = 1; // Înlocuitor pentru 'id'
    for (const record of records) {
      const { name, detailed_name } = record;
      ...
      // Creăm un text descriptiv care include name, detailed_name, și iataCode
      const textToProcess = `${name || "Airport"} - ${
        detailed_name || "No detailed name"
      }`.trim();
      ...
      for (const chunk of chunks) {
        ...
        const res = await collection.insertOne({
          ...
          metadata: {
            ...
            source: "neon_db",
            name: name || "Unknown",
            detailed_name: detailed_name || "N/A",
          },
        });
