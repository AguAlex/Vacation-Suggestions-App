...
const createCollection = async (
  ...
  try {
    const res = await db.createCollection(ASTRA_DB_COLLECTION_5, {
      vector: {
...
const loadSampleData = async () => {
  try {
    const collection = await db.collection(ASTRA_DB_COLLECTION_5);

    // Conectare la baza de date Neon
    await neonClient.connect();
    console.log("Connected to Neon database");

    // Verifică schema tabelului cities
    const schemaQuery =
      "SELECT column_name, data_type FROM information_schema.columns WHERE table_name = 'cities'";
    const schemaResult = await neonClient.query(schemaQuery);
    console.log("Cities table schema:", schemaResult.rows);

    // Verifică schema tabelului countries
    const schemaQueryCountries =
      "SELECT column_name, data_type FROM information_schema.columns WHERE table_name = 'countries'";
    const schemaResultCountries = await neonClient.query(schemaQueryCountries);
    console.log("Countries table schema:", schemaResultCountries.rows);

    // Extrage datele din tabelul Neon
    const sourceQuery =
      "SELECT s.name AS city_name, c.name AS country_name FROM cities s LEFT JOIN countries c ON s.country_id = c.id";
    ...
    if (records.length > 0) {
      ...
      console.warn("No records found in cities table");
      return;
    }

    // Pentru fiecare înregistrare, procesăm city_name și country_name
    let recordIndex = 1; // Înlocuitor pentru 'id'
    for (const record of records) {
      const { city_name, country_name } = record;
      ...
      // Creăm un text descriptiv care include city_name și country_name
      const textToProcess = `${city_name || "City"} - ${
